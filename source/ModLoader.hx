...
class ModLoader
{
	...
	public static function getFreeplaySongs()
	{
		...
		for(i in weekList)
		{
			var songData:Array<String> = File.getContent('mods/weeks/${weekList[i]}/freeplaySonglist.txt').trim().split('\n');
	...
	public static function getFreeplayFolders()
	{
		...
		for(i in weekList)
		{
			for(g in 0...(File.getContent('mods/weeks/${weekList[i]}/freeplaySonglist.txt')).length)
			{
